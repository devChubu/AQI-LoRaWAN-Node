 ...
 *   - Adafruit_PM25AQI.h
 * Usage: 
 *   - This code initializes various sensors and modules, reads data from them, 
 *     calculates AQI, and sends the data via LoRa.
 */
...
#define NODE_ID "Node 2" // Create variable for Node ID
...
void setup() {
  ...
  if (rtc.begin() == false) {
    ...
    // Blink LED rapidly to indicate RTC initialization failure
    while (1){
  ...
  // Blink LED once to indicate successful RTC initialization
  digitalWrite(LED_BUILTIN, HIGH);
  delay(500);
  digitalWrite(LED_BUILTIN, LOW);
  delay(500);
  ...
  if (!sht4.begin()) {
    ...
    // Blink LED rapidly to indicate SHT45 initialization failure
    while (1){
      digitalWrite(LED_BUILTIN, HIGH);
      delay(100);
      digitalWrite(LED_BUILTIN, LOW);
  ...
  // Blink LED twice to indicate successful SHT45 initialization
  for (int i = 0; i < 2; i++) {
    digitalWrite(LED_BUILTIN, HIGH);
    delay(500);
    digitalWrite(LED_BUILTIN, LOW);
  ...
  // Blink LED rapidly to indicate PMSA003I initialization failure
  while (1){
      digitalWrite(LED_BUILTIN, HIGH);
      delay(100);
      digitalWrite(LED_BUILTIN, LOW);
  ...
  // Blink LED three times to indicate successful PMSA003I initialization
  for (int i = 0; i < 3; i++) {
    digitalWrite(LED_BUILTIN, HIGH);
    delay(500);
    digitalWrite(LED_BUILTIN, LOW);
  ...
  if (!LoRa.begin(923200000)) { //Set frequency for Philippines
    ...
    // Blink LED rapidly to indicate LoRa initialization failure
    while (1){
      digitalWrite(LED_BUILTIN, HIGH);
      delay(100);
      digitalWrite(LED_BUILTIN, LOW);
  ...
  // Blink LED four times to indicate successful LoRa initialization
  for (int i = 0; i < 4; i++) {
    digitalWrite(LED_BUILTIN, HIGH);
    delay(500);
    digitalWrite(LED_BUILTIN, LOW);
